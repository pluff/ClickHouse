...
def main():
    ...
    if res and build_type in BUILD_TYPE_TO_STATIC_LOCATION:
        if (
            ...
            def do():
                S3.copy_file_to_s3(
                    local_path=f"{temp_dir}/build/clickhouse",
                    s3_path=f"{S3_BUCKET_NAME}/master/{BUILD_TYPE_TO_STATIC_LOCATION[build_type]}/clickhouse-full",
                )
                S3.copy_file_to_s3(
                    local_path=f"{temp_dir}/build/clickhouse-stripped",
                    s3_path=f"{S3_BUCKET_NAME}/master/{BUILD_TYPE_TO_STATIC_LOCATION[build_type]}/clickhouse",
                )
